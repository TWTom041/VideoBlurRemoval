...
with torch.autocast("cuda", torch.bfloat16):
    ...
    for vid_folder in pathlib.Path("VSPW/data/").iterdir():
        if f"{vid_folder.name}.pt" in pathlib.Path(OUT_FOLDER).iterdir():
        ...
        torch.save(latent, pathlib.Path(OUT_FOLDER) / f"{vid_folder.name}.pt")
